...
local evidenceMarkers = {
	...
	fingerprint = { label = "Fingerprint", marker = 25, r = 200, g = 200, b = 0, a = 155, scale = 0.15, textZOffset = 0.25}
}
				...
				if CanSeeEvidence() then
					for k, v in ipairs(nearbyEvidence) do
						if #(GetEntityCoords(LocalPlayer.state.ped) - v.coords) <= 8.0 then
							...
							if v.type == "fingerprint" then
								text = string.format("Fingerprint | %d%%", v.data.quality)
							end
